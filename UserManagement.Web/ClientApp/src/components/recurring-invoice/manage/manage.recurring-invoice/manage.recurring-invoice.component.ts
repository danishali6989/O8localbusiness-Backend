...
@Component({
  ...
  styleUrls: ['./manage.recurring-invoice.component.css']
})
export class ManageRecurringInvoiceComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.dtOptions = {
          ...
          ajax: (dataTablesParameters: any, callback) => {
              ...
              self.http
                  .post<DataTableResponseModel>(this.appSettings.ApiBaseUrl + 'RecurringInvoice/paged-result', dataTablesParameters, {})
                  .subscribe(resp => {
                      callback({
          ...
          rowCallback: function (row, data: any) {
              ...
              $(editElem).on('click', function () {
                  self.router.navigate(['/recurring-invoice/edit', data.id]);
              });
              ...
              $(detailElem).on('click', function () {
                  self.router.navigate(['/recurring-invoice/detail', data.id]);
              });

              const payElem = $(row).find('[action-type = pay-invoice]');
              $(payElem).unbind('click');
              $(payElem).on('click', function () {
                  self.router.navigate(['/recurring-invoice/payment', data.id]);
              });
          },
