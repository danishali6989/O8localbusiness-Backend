...
export class PurchaseVendorsService {
  
  constructor(private http: HttpClient,
    private appSettings: AppSettings) { }

  add(model: PurchaseVendorsDetail) {
    return this.http.post(this.appSettings.ApiBaseUrl + 'Report/Add', model);
  }


  getVendorStatement(purVendorDetail){
    debugger;

    return this.http.post(this.appSettings.ApiBaseUrl + 'Report/vendor_report_details', purVendorDetail);
  }
